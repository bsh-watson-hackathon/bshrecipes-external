...
function convIngredList2BSH(data){
  let ListIngredients=[];
  let currentlist = {name:"ingredients", ingredients:[] };
  for(let a = 0; (a < data.length);a++) {
    if((data[a].type == "tag") && (data[a].name == "li")) {
        if((data[a].attribs) && (data[a].attribs.class === "ingred-heading")) {
          ...
          currentlist = {name:normIngred(data[a].children[0].raw),ingredients:[]};
        } else {
          currentlist.ingredients.push( normIngred(data[a].children[0].raw) );
        }
    }
  }
  if(currentlist.ingredients.length > 0){
    ListIngredients.push(currentlist)
  }
